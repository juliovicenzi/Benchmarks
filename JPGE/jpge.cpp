...
//sets the values of the object to 0
template <class T> inline void clear_obj(T &obj)
{
    memset(&obj, 0, sizeof(obj));
}
//functions that convert from YCbCr not sure if I should mess with it?
...
// Forward DCT
//essa função é chamada varias vezes dentro de um loop, então seria interessante optimiza-la
//problema: não temos como vetorizar tamanhos de float de 64 bits
...
#else
    float32x4_t data0_3, data7_4, tmp_low, tmp_high, znum;
    float32x4_t temp_const, z_const; //holds constants for parallel multiplication
    const dct_t _tmp_const[4] = {1.501321110, 3.072711026, 2.053119869, 0.298631336};
    temp_const = vld1q_f32(_tmp_const);
    const dct_t _z_const[4] = {-0.899976223, -2.562915447, -1.961570560, -0.390180644};
    z_const = vld1q_f32(_z_const);
    for (int c=0; c < 8; c++) {
        data0_3 = vld1q_f32(&data_ptr[0]); //hold data values from data_ptr[0] to data_ptr[3]
        data7_4;    //holds values from data_ptr[7] downto data_ptr[4]
        //we must use this element order to make parallel instructions usable
        data7_4[0] = data_ptr[7];
        data7_4[1] = data_ptr[6];
        data7_4[2] = data_ptr[5];
        data7_4[3] = data_ptr[4];
        tmp_low = vaddq_f32(data0_3, data7_4); //holds values of tmp0 to tmp3
        tmp_high = vsubq_f32(data0_3, data7_4); //holds values of tmp7 downto tmp4
        tmp10 = tmp_low[0] + tmp_low[3];
        tmp13 = tmp_low[0] - tmp_low[3];
        tmp11 = tmp_low[1] + tmp_low[2];
        tmp12 = tmp_low[1] - tmp_low[2];
        data_ptr[0] = tmp10 + tmp11;
        data_ptr[4] = tmp10 - tmp11;
        znum[0] = (tmp12 + tmp13) * 0.541196100;
        data_ptr[2] = znum[0] + tmp13 * 0.765366865;
        data_ptr[6] = znum[0] + tmp12 * - 1.847759065;
        znum[0] = tmp_high[3] + tmp_high[0];
        znum[1] = tmp_high[2] + tmp_high[1];
        znum[2] = tmp_high[3] + tmp_high[1];
        znum[3] = tmp_high[2] + tmp_high[0];
        z5 = (znum[2] + znum[3]) * 1.175875602;
        tmp_high = vmulq_f32(temp_const, tmp_high);
        znum = vmulq_f32(z_const, znum);
        znum[2] += z5;
        znum[3] += z5;
        data_ptr[7] = tmp_high[3] + znum[0] + znum[2];
        data_ptr[5] = tmp_high[2] + znum[1] + znum[3];
        data_ptr[3] = tmp_high[1] + znum[1] + znum[2];
        data_ptr[1] = tmp_high[0] + znum[0] + znum[3];
        data_ptr += 8;
    }
    data_ptr = data;
    for (int c=0; c < 8; c++) {
        data0_3[0] = data_ptr[8*0];
        data0_3[1] = data_ptr[8*1];
        data0_3[2] = data_ptr[8*2];
        data0_3[3] = data_ptr[8*3];
        data7_4[0] = data_ptr[8*7];
        data7_4[1] = data_ptr[8*6];
        data7_4[2] = data_ptr[8*5];
        data7_4[3] = data_ptr[8*4];
        tmp_low = vaddq_f32(data0_3, data7_4);
        tmp_high = vsubq_f32(data0_3, data7_4);
        tmp10 = tmp_low[0] + tmp_low[3];
        tmp13 = tmp_low[0] - tmp_low[3];
        tmp11 = tmp_low[1] + tmp_low[2];
        tmp12 = tmp_low[1] - tmp_low[2];
        ...
        data_ptr[8*4] = (tmp10 - tmp11) / 8.0;
        znum[0] = (tmp12 + tmp13) * 0.541196100;
        data_ptr[8*2] = (znum[0] + tmp13 * 0.765366865) / 8.0;
        data_ptr[8*6] = (znum[0] + tmp12 * -1.847759065) / 8.0;
        znum[0] = tmp_high[3] + tmp_high[0];
        znum[1] = tmp_high[2] + tmp_high[1];
        znum[2] = tmp_high[3] + tmp_high[1];
        znum[3] = tmp_high[2] + tmp_high[0];
        ...
        znum[2] += z5;
        znum[3] += z5;
        data_ptr[8*7] = (tmp_high[3] + znum[0] + znum[2]) / 8.0;
        data_ptr[8*5] = (tmp_high[2] + znum[1] + znum[3]) / 8.0;
        data_ptr[8*3] = (tmp_high[1] + znum[1] + znum[2]) / 8.0;
        data_ptr[8*1] = (tmp_high[0] + znum[0] + znum[3]) / 8.0;
        data_ptr++;
    }
#endif
}
...
static void huffman_enforce_max_code_size(int *pNum_codes, int code_list_len, int max_code_size)
{
    ...
    #else
    int32x4_t temp_sum = vmovq_n_s32(0); //starts with 0
    for (int i = max_code_size + 1; i <= MAX_HUFF_CODESIZE; i+=4) {
        temp_sum = vaddq_s32(temp_sum, vld1q_s32(&pNum_codes[i]));
    }
    //we now add every parallel sum
    pNum_codes[max_code_size] += temp_sum[0] + temp_sum[1] + temp_sum[2] + temp_sum [3];
    const int _shifted_indices[4] = {3, 2, 1, 0};
    const int32x4_t shifted_indices = vld1q_s32(_shifted_indices); //constant to make quickly set shift amounts
    uint32x4_t total_vector = vdupq_n_u32(0); //inicializamos o vetor com todos valores zerados
    for (int i = max_code_size; i > 0; i-=4){
        uint32x4_t pNum_temp = vreinterpretq_u32_s32(vld1q_s32(&pNum_codes[i-3]));
        int32x4_t shift_amount = vdupq_n_s32(max_code_size - i);
        shift_amount = vaddq_s32(shifted_indices, shift_amount);
        total_vector = vaddq_u32(total_vector, vshlq_u32(pNum_temp, shift_amount));
    }
    //we add everything back into total as a single variable
    uint32 total = total_vector[0] + total_vector[1] + total_vector[2] + total_vector[3];
    #endif
...
void image::load_block(dct_t *pDst, int x, int y)
{
    ...
    #else
    //vectorization possivel se dct_t não fosse double edit: mudamos dtc_t para float
    uint8x8_t DstTemp;
    for (int i = 0; i < 8; i++, pDst += 8) {
        vst1q_f32(pDst, vld1q_f32( &m_pixels[(y+i)*m_x + x]));
        vst1q_f32(pDst + 4, vld1q_f32(&m_pixels[(y+i)*m_x + x + 4]));
    }
    #endif
}
