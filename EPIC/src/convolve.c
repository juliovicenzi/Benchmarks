...
internal_filter(float * image, int x_dim, int y_dim, float *filt, float *temp, int x_fdim, int y_fdim,
  ...
  //vec variables
  #ifdef opt
  float32x4_t v_image, v_temp, v_sum;
  #endif
  if (!reflect) 
      {
  ...
  //LOOPDEF=NESTED_DYNAMIC  vec
  printf("Primeiro loop\n");
  for (y_pos=ygrid_start-y_fmid-1,res_pos=0;y_pos<0;y_pos+=ygrid_step)
  {
    for (x_pos=xgrid_start-x_fmid-1;x_pos<0;x_pos+=xgrid_step,res_pos++)
    {
      (*reflect)(filt,x_fdim,y_fdim,x_pos,y_pos,temp,FILTER);
      sum=0.0; 
      for (y_filt_lin=x_fdim,x_filt=y_im_lin=0; y_filt_lin<=filt_size;y_im_lin+=x_dim,y_filt_lin+=x_fdim)
      {    
          //arm
          #ifndef opt
          for (im_pos=y_im_lin;x_filt<y_filt_lin;x_filt++,im_pos++)
            sum+=image[im_pos]*temp[x_filt];
          //neon instrinsics
          #else
          v_sum = vdupq_n_f32(0);
          for (im_pos=y_im_lin; x_filt<(y_filt_lin - (y_filt_lin%4)); x_filt+=4,im_pos+=4)
          {
            v_image = vld1q_f32(&image[im_pos]);
            v_temp = vld1q_f32(&temp[x_filt]);
            v_sum = vmlaq_f32(v_sum, v_image, v_temp);
          }
          for (; x_filt<y_filt_lin; x_filt++,im_pos++)
            sum+=image[im_pos]*temp[x_filt];
          sum += v_sum[0] + v_sum[1] + v_sum[2] + v_sum[3];
          #endif
          printf("x_filt: %d, y_filt_lin: %d\n", x_filt, y_filt_lin);
          //TODO x_filt with wrong values
      }
      printf("%d \n", sum);
      result[res_pos] = sum;
    }
    ...
    for (x_pos=first_col;x_pos<last_ctr_col;x_pos+=xgrid_step,res_pos++) 
	  {
      sum=0.0;
      for (y_filt_lin=x_fdim,x_filt=y_im_lin=0;y_filt_lin<=filt_size;y_im_lin+=x_dim,y_filt_lin+=x_fdim)
      {
        // //arm
        // #ifndef opt
        for (im_pos=x_pos+y_im_lin;x_filt<y_filt_lin;x_filt++,im_pos++)
          sum+=image[im_pos]*temp[x_filt];
        //neon instrinsics
        // #else
        // v_sum = vdupq_n_f32(0);
        // for (im_pos=x_pos+y_im_lin; x_filt<(y_filt_lin - (y_filt_lin%4)); x_filt+=4,im_pos+=4)
        // {
        //   v_image = vld1q_f32(&image[im_pos]);
        //   v_temp = vld1q_f32(&temp[x_filt]);
        //   v_sum = vmlaq_f32(v_sum, v_image, v_temp);
    ...
    for (x_pos+=(1-last_ctr_col); x_pos<x_stop;x_pos+=xgrid_step,res_pos++) 
	  {
      (*reflect)(filt,x_fdim,y_fdim,x_pos,y_pos,temp,FILTER);
      sum=0.0;
      for (y_filt_lin=x_fdim,x_filt=y_im_lin=0;y_filt_lin<=filt_size;y_im_lin+=x_dim,y_filt_lin+=x_fdim)
      {    
        // //arm
        // #ifndef opt
        for (im_pos=y_im_lin+last_ctr_col;x_filt<y_filt_lin;x_filt++,im_pos++)
          sum+=image[im_pos]*temp[x_filt];
        // //neon instrinsics
        // #else
        // v_sum = vdupq_n_f32(0);
        // for (im_pos=y_im_lin+last_ctr_col; x_filt<(y_filt_lin - (y_filt_lin%4)); x_filt+=4,im_pos+=4)
        // {
        //   v_image = vld1q_f32(&image[im_pos]);
        //   v_temp = vld1q_f32(&temp[x_filt]);
        //   v_sum = vmlaq_f32(v_sum, v_image, v_temp);
  ...
  prev_res_pos = res_pos;
   //LOOPDEF=NESTED_DYNAMIC  vec
  for (x_pos=xgrid_start-x_fmid-1;x_pos<0;x_pos+=xgrid_step)
  {
    res_pos = prev_res_pos;
    (*reflect)(filt,x_fdim,y_fdim,x_pos,0,temp,FILTER);
    for (y_pos=first_row;y_pos<last_ctr_row;y_pos+=ygrid_step_full, res_pos+=x_res_dim)
	  {
      sum=0.0;
      for (y_filt_lin=x_fdim,x_filt=0,y_im_lin=y_pos;y_filt_lin<=filt_size;y_im_lin+=x_dim,y_filt_lin+=x_fdim)
        for (im_pos=y_im_lin;x_filt<y_filt_lin;x_filt++,im_pos++)
          sum+=image[im_pos]*temp[x_filt];
      result[res_pos] = sum;
	  }
  ...
  prev_res_pos = rt_edge_res_pos;
   //LOOPDEF=NESTED_DYNAMIC  vec
  for (x_pos+=(1-last_ctr_col);x_pos<x_stop;x_pos+=xgrid_step) 
  {
    res_pos = prev_res_pos;
    (*reflect)(filt,x_fdim,y_fdim,x_pos,0,temp,FILTER);
    for (y_pos=first_row;y_pos<last_ctr_row;y_pos+=ygrid_step_full, res_pos+=x_res_dim)
  ...
  res_pos -= (x_res_dim - 1);            /* go to lower left corner */
   //LOOPDEF=NESTED_DYNAMIC  vec
  for (y_pos=((y_pos-last_ctr_row)/x_dim)+1; y_pos<y_stop;y_pos+=ygrid_step) 
  {
    for (x_pos=xgrid_start-x_fmid-1;x_pos<0;x_pos+=xgrid_step,res_pos++)
	  {
      (*reflect)(filt,x_fdim,y_fdim,x_pos,y_pos,temp,FILTER);
      sum=0.0;
      for (y_filt_lin=x_fdim,x_filt=0,y_im_lin=last_ctr_row;y_filt_lin<=filt_size;y_im_lin+=x_dim,y_filt_lin+=x_fdim)
        for (im_pos=y_im_lin;x_filt<y_filt_lin;x_filt++,im_pos++)
          sum+=image[im_pos]*temp[x_filt];
      result[res_pos] = sum;
	  }
