...
#ifdef opt
#include <arm_neon.h>   // arm neon optimization
#endif
...
float temp;
#ifdef opt
float32x4_t v_temp, v_k;
float inv_thresh = 1 / ((float) thresh);  //we use 1/x to invert tresh and use multiplication instruction
float32x4_t v_inv_thresh = vdupq_n_f32(inv_thresh);
float32x4_t v_four = vdupq_n_f32(4);    //sets constant 4 vector. to increment the v_k vector
float32x4_t v_hundred = vdupq_n_f32(100);  //constant 100 vector
v_k[0] = -256;  //sets starting values of k vector
v_k[1] = -255;
v_k[2] = -254;
v_k[3] = -253;
  ...
  *bp=*bp+258;
  //this loop ALWAYS runs 513 times. 
  // 
  
  for(k=-256;k<256;k+=4)
  {
    v_temp = vmulq_f32(v_k, v_inv_thresh);
    v_temp = vmulq_f32(v_temp, v_temp);
    if (form==6)
      v_temp = vmulq_f32(vmulq_f32(v_temp, v_temp), v_temp);
    v_temp[0] = exp(-v_temp[0]);
    v_temp[1] = exp(-v_temp[1]);
    v_temp[2] = exp(-v_temp[2]);
    v_temp[3] = exp(-v_temp[3]);

    v_temp = vmulq_f32(v_temp, v_hundred);
    //erro atual: segmentation fault D:
    //não sei como guardar isso em paralelo
    *(*bp+k)= (uchar) v_temp[0];
    *(*bp+k+1)= (uchar) v_temp[1];
    *(*bp+k+2)= (uchar) v_temp[2];
    *(*bp+k+3)= (uchar) v_temp[3];
    //idealmente usariamos isto, mas não estou conseguindo fazer funcionar
    //vst1q_u8(*(*bp + k), vreinterpretq_u8_f32(v_temp)); //isso n funciona por alguma razão
    v_k = vaddq_f32(v_k, v_four);   //increments the v_k vector
  }
 //we can't paralelize the last iteration:
  temp= 256 / ((float)thresh);
  temp=temp*temp;
  if (form==6)
    temp=temp*temp*temp;
  temp=100.0*exp(-temp);
...
uchar *p,*cp;
  ...
  memset (r,0,x_size * y_size * sizeof(int));
    //provavelemten MUITO VECTORIZATION
...
/* }}} */
  ...
  if (three_by_three==0)
    ...
    mask_size = 1;    //masksize = 1 is a 3x3 mask
...
/* }}} */
  ...
  #ifdef opt
    int32x4_t v_x;
    float32x4_t v_y; //temporary vector for intermediary operations
    float32x4_t v_four = vdupq_n_f32(4); //4 constant for increment of v_j
    float32x4_t v_one = vdupq_n_f32(1); //1 constant for increment of v_i
    float32x4_t v_hundred = vdupq_n_f32(100); //100 constant for multiplication
    float32x4_t v_inv_temp = vdupq_n_f32(1/temp); //this is necessary for vector division
    float32x4_t v_i, v_j;
    //sets inital values of v_j and v_i to -mask_size
    v_i[0] = (float) -mask_size;
    v_i[1] = (float) (-mask_size+1);
    v_i[2] = (float) (-mask_size+2);
    v_i[3] = (float) (-mask_size+3);
    v_j[0] = (float) -mask_size;
    v_j[1] = (float) (-mask_size+1);
    v_j[2] = (float) (-mask_size+2);
    v_j[3] = (float) (-mask_size+3);

  //vectorize:
    //Verificamos o tamanho de masksize, vetorizamos iterações em multiplos de 4
    int v_loop_max = n_max % 4;   //number of iterations that cannot be vectorized
    //int count = 0;
    for(i=-mask_size; i<=mask_size; i++)
    {
      for(j=-mask_size; j<=mask_size-v_loop_max; j+=4)  //inc de +4!!
      { //count ++;
        v_y = vmulq_f32(v_i, v_i);
        v_y = vmulq_f32(v_y, vmulq_f32(v_j, v_j));
        v_y = vmulq_f32(v_y, v_inv_temp); // ((float)((i*i)+(j*j))) / temp )
        v_y[0] = exp(v_y[0]);
        v_y[1] = exp(v_y[1]);
        v_y[2] = exp(v_y[2]);
        v_y[3] = exp(v_y[3]);
        v_x = vreinterpretq_s32_f32(vmulq_f32(v_y, v_hundred));
        //x = (int) (100.0 * exp( ((float)((i*i)+(j*j))) / temp ));
        //*dpt++ = (unsigned char)x;  //this puts x in dpt address, then increments dpt by 1
        vst1q_u8(dpt, vreinterpretq_u8_s32(v_x));
        v_j = vaddq_f32(v_j, v_four); //j+=4
        *dpt+=4;
      }
      v_i = vaddq_f32(v_i, v_one); // i++
    }  //we cant vectorize this:
  for(i=-mask_size; i<=mask_size; i++)
    for(j=mask_size-(v_loop_max) + 1; j<=mask_size; j++)
    {//count++;
      x = (int) (100.0 * exp( ((float)((i*i)+(j*j))) / temp ));
      *dpt++ = (unsigned char)x;
    }
...
/* }}} */
    /* {{{ main section */
#ifdef opt
  int32x4_t v_brightness, v_area, v_total, v_tmp, v_cp, v_dpt;
  for (i=mask_size;i<y_size-mask_size;i++)
  {
    for (j=mask_size;j<x_size-mask_size;j++)
    {
      ...
      {
        v_total = vdupq_n_s32(0); 
        v_area = vdupq_n_s32(0);
        for(x=-mask_size; x<=mask_size-(n_max%4); x+=4)
	      {
          v_brightness[0] = *(ip);
          v_brightness[1] = *(ip+1);
          v_brightness[2] = *(ip+2);
          v_brightness[3] = *(ip+3);
          //instrução de load n funciona direito por alguma razão ??
          v_cp[0] = *(cp-*ip++);
          v_cp[1] = *(cp-*ip++);
          v_cp[2] = *(cp-*ip++);
          v_cp[3] = *(cp-*ip++);
          ...
          v_dpt[2] = *dpt++;
          v_dpt[3] = *dpt++;
          v_tmp = vmulq_s32(v_dpt, v_cp);
          v_area = vaddq_s32(v_area, v_tmp);
          v_total = vaddq_s32(v_total, vmulq_s32(v_tmp, v_brightness));
        }
        //we cannot vectorize this part:
        area  += v_area[0] + v_area[1] + v_area[2] + v_area[3];
        total += v_total[0] + v_total[1] + v_total[2] +v_total[3];
        for(x=mask_size-(n_max%4)+1; x<=mask_size; x++)
	      {
          brightness = *ip++;   //coloca ip em brightness e dps incrementa ip
          tmp = *dpt++ * *(cp-brightness);  //cp constant during um loop
          area += tmp;
          total += tmp * brightness;    //mult e incremento !!
        }
        ip += increment;
      }
      tmp = area-10000;
