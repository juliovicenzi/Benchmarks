...
static void sha_transform(SHA_INFO *sha_info)
{
    ...
    uint32x4_t aux;
    #endif
    //LOOPDEF=COUNT vec (very little gain)
    //ESSE LOOP AQUI JONATHAN
    #ifndef opt
    for (i = 0; i < 16; ++i) 
    {
        W[i] = sha_info->data[i];
    ...
    for (i = 0; i < 16; i+=4)
    {
        aux = vld1q_u32((const unsigned int*) (&(sha_info->data[i])));
        vst1q_u32((unsigned int*)(&W[i]), aux);
    }
    #endif
    //LOOPDEF=COUNT  vec*** (but no way to make this kind of XOR calculation regarding this indexes)
    for (i = 16; i < 80; ++i) {
